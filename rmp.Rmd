...
```{r, message=FALSE, warning=FALSE, echo=TRUE} 
sumCoef<-summary(fit5)$coefficients 
...
The fact that the Confidence interval contains, negative, zero and positive values means that **I cannot define**
-1) whether mpg increases or decreases.
-2) how much it will either increase or decrease.

This with the model including other variables.
...
coefficients2 <-sumCoef[1,1]+c(-1,1)*qt(.975,df=fam$df)*sumCoef[1,2]
```
With 95% confidence a manual transmission gives **`r coefficients2[1]`** to **`r coefficients2[2]`** miles per gallon more than an automatic transmission.
