...
From the coefficients it occurs that a manual transmission **increases the efficiency of miles per gallon by `r round(f6sincrease,2) `  with a standard error of `r round(f6serror,2) ` **. 
...
With 95% confidence this is quantified by getting predicted values from our model (fit6)
...
```
The results move in the zone of **`r round(coefficients[1],2) `** to  **`r round(coefficients[2],2)`**.
